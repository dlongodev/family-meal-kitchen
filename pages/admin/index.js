...
export const getServerSideProps = async (ctx) => {
    ...
    const [menuRes, orderRes, catRes] = await Promise.all([
        axios.get(`${process.env.PUBLIC_NEXT_VERCEL_URL}/api/menu`),
        axios.get(`${process.env.PUBLIC_NEXT_VERCEL_URL}/api/orders`),
        axios.get(`${process.env.PUBLIC_NEXT_VERCEL_URL}/api/category`)
    ]);
    return {
        props: {
            menu: menuRes.data,
